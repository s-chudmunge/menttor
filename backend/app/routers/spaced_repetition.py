...
def get_all_spaced_repetition_entries(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    entries = db.exec(select(SpacedRepetition).where(SpacedRepetition.user_id == current_user.id)).all()
    return entries
...
def get_spaced_repetition_entry_by_id(entry_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    entry = db.exec(select(SpacedRepetition).where(SpacedRepetition.id == entry_id, SpacedRepetition.user_id == current_user.id)).first()
    if not entry:
