      ...
      {modules.map((module: any) => {
        ...
        return (
          <div key={module.id} className="group relative">
            ...
            <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-lg rounded-3xl border border-white/20 dark:border-gray-700/50 shadow-2xl p-6 lg:p-10 transition-all duration-300 hover:shadow-3xl">
              {/* Module header */}
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 lg:mb-8 space-y-4 sm:space-y-0">
                <div className="flex-1">
                  <h3 className="text-xl lg:text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center space-x-3">
                    <div className="w-3 h-3 lg:w-4 lg:h-4 bg-indigo-500 rounded-full shadow-lg"></div>
              ...
              <div className="relative">
                ...
                <div className="absolute left-4 lg:left-6 top-0 bottom-0 w-0.5 bg-indigo-500/30 dark:bg-indigo-400/40"></div>
                
                <div className="space-y-8 lg:space-y-10">
                  {module.topics.map((topic: any, topicIndex: number) => (
                    <div key={topic.id} className="relative pl-10 lg:pl-14">
                      {/* Modern topic node */}
                      <div className="absolute left-2 lg:left-4 top-2 w-5 h-5 lg:w-6 lg:h-6 bg-indigo-500 border-2 border-white dark:border-gray-800 rounded-full shadow-lg flex items-center justify-center">
                        <div className="w-2 h-2 lg:w-2.5 lg:h-2.5 bg-white rounded-full"></div>
                      </div>
                      
                      {/* Topic content */}
                      <div className="bg-white/80 dark:bg-gray-700/80 backdrop-blur-sm rounded-2xl border border-white/30 dark:border-gray-600/30 shadow-lg p-6 lg:p-8 transition-all duration-300 hover:shadow-xl">
                          ...
                          {topic.subtopics.map((subtopic: any) => {
                            ...
                            return (
                              <div key={subtopic.id} className="group/card relative">
                                ...
                                <div className="relative bg-white/90 dark:bg-gray-700/90 backdrop-blur-sm rounded-xl border border-white/40 dark:border-gray-600/40 shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300 overflow-hidden">
                                  {/* Status indicator */}
                                  <div className={`absolute top-0 left-0 w-full h-1 ${status === 'completed' ? 'bg-green-500' : status === 'ready-for-quiz' ? 'bg-yellow-500' : status === 'learning' ? 'bg-blue-500' : 'bg-gray-300'}`}></div>
                                  ...
                                  <div className="p-5 lg:p-6">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {subtopic.has_learn && (
                                        ...
                                        >
                                          <div className="absolute inset-0 bg-white/0 hover:bg-white/20 transition-colors duration-300"></div>
                                      ...
                                      {subtopic.has_code_challenge && (
                                        <button
                                          ...
                                          className="group/btn relative overflow-hidden bg-cyan-600 hover:bg-cyan-700 text-white px-3 py-2 rounded-lg font-medium text-xs lg:text-sm transition-all duration-300 shadow-md hover:shadow-lg"
                                        >
                                          <div className="absolute inset-0 bg-white/0 hover:bg-white/20 transition-colors duration-300"></div>
                                          <div className="relative flex items-center space-x-1">
